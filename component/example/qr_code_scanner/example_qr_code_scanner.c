	...
	if (cml->voe == 1) {
		...
		if ((enc2out->codec & (CODEC_H264 | CODEC_HEVC | CODEC_JPEG)) != 0) {
			...
			if (yuv_ctx->snap_flag) {
				...
				yuv_ctx->snap_flag = 0;
				yuv_ctx->isp_len = enc2out->enc_len;
			} else {
...
void yuv_snapshot_init(void *ctx)
{
	...
	printf("\r\n voe heap size = %d\r\n", voe_heap_size);
	video_alloc_heap(yuv_ctx->v_adp, voe_heap_size);
...
{
	struct yuv_snapshot_context *yuv_ctx = (struct yuv_snapshot_context *)ctx;
	video_open(yuv_ctx->v_adp, &video_v1_params, yuv_output_cb, (void *)ctx);
}
...
unsigned char *yuv_snapshot_get(void *ctx)
{
	struct yuv_snapshot_context *yuv_ctx = (struct yuv_snapshot_context *)ctx;
	yuv_ctx->snap_flag = 1;
	video_ctrl(yuv_ctx->v_adp, video_v1_params.stream_id, VIDEO_NV12_OUTPUT, 1);
...
{
	struct yuv_snapshot_context *yuv_ctx = (struct yuv_snapshot_context *)ctx;
	video_close(yuv_ctx->v_adp, video_v1_params.stream_id);
}
...
static void example_qr_code_scanner_thread(void *param)
{
	...
	vTaskDelay(5000);//Wait wifi init for voe problem
	...
	while (1) {
		...
		video_ctrl(yuv_ctx->v_adp, video_v1_params.stream_id, VIDEO_NV12_OUTPUT, 0);
