...
static STATUS handleOffer(PAppConfiguration pAppConfiguration, PStreamingSession pStreamingSession, PSignalingMessage pSignalingMessage)
{
	...
	if (pStreamingSession->receiveAudioVideoSenderTid == INVALID_TID_VALUE) {
		runMediaReceive(pStreamingSession);
	}
#endif
...
STATUS freeStreamingSession(PStreamingSession *ppStreamingSession)
{
	...
	if (IS_VALID_TID_VALUE(pStreamingSession->receiveAudioVideoSenderTid)) {
		THREAD_CANCEL(pStreamingSession->receiveAudioVideoSenderTid);
		pStreamingSession->receiveAudioVideoSenderTid = INVALID_TID_VALUE;
	}
#endif
