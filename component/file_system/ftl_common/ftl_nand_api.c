...
int make_incorrect_data = 0;
#endif

bbm_info_attr *bbm_info = NULL;

int nand_num_swap(unsigned char *ptr)
{
...
int ftl_read_nand_fw_info(bbm_info_attr *bbm_info)
{
	unsigned char *fw_info = malloc(2048 + 32);
	int page_default_per_blk = 64;
	int page_size = 2048;
	int ret = -1;
	if (fw_info == NULL) {
		printf("Fw_info malloc failed\r\n");
		goto EXIT;
	}
	for (int i = 0; i < 6; i++) {
		ret = bbm_info->nand_read_cb((FW_INFO_BLOCK_INDEX + i) * (page_size * page_default_per_blk), 2048 + 32, fw_info);
		if (ret < 0) {
			printf("snand_page_read fail %d\r\n", i);
		} else {
			bbm_info->blk_cnt = nand_num_swap(fw_info);
			bbm_info->page_per_blk = nand_num_swap(fw_info + PAGE_PER_BLK_INDEX);
			bbm_info->page_size = nand_num_swap(fw_info + PAGE_SIZE_INDEX);
			bbm_info->spare_size = nand_num_swap(fw_info + SPARE_SIZE_INDEX);
			if (bbm_info->page_size <= 2048) {
				if (fw_info[bbm_info->page_size] == 0xff) {
					if (fw_info[bbm_info->page_size + 1] == 0x87 && fw_info[bbm_info->page_size + 2] == 0XFF && fw_info[bbm_info->page_size + 3] == 0X35 &&
						fw_info[bbm_info->page_size + 4] == 0XFF) {
						printf("The fw tag is correct\r\n");
						if ((bbm_info->blk_cnt != 1024 && bbm_info->blk_cnt != 2048) || bbm_info->page_per_blk != 64 || bbm_info->page_size != 2048) {
							printf("The block cnt is not correct %d page size %d page_per_blk %d\r\n", bbm_info->blk_cnt, bbm_info->page_size, bbm_info->page_per_blk);
						} else {
							ret = 0;
							break;
						}
					...
					printf("The block is broken %d\r\n", i);
				}
			} else {
				printf("The page size is not support\r\n", bbm_info->page_size);
			}
		}
	}
	if (fw_info) {
...
int ftl_init_nand(void)
{
	...
	if (bbm_info == NULL) {
		...
		ret = ftl_read_nand_fw_info(bbm_info);
		if (ret < 0) {
			printf("It can't find the nand flash info\r\n");
			goto EXIT;
		}
