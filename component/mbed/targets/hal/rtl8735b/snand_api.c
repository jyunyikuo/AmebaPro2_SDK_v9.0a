...
hal_snafc_adaptor_t *pglob_snafc_adaptor = NULL;
 ...
 */
#define SUPPORT_QUAD_BIT_MODE_R (0)
/**
 0: WRITE page data via 1-bit mode (default)
 1: WRITE page data via 4-bit mode
 */
#define SUPPORT_QUAD_BIT_MODE_W (0)
...
hal_snafc_adaptor_t mMBedAmebaPro2SnafcAdaptor = {
	...
	.initVal[SNAFC_SPEED_SEL_0].l = 0x00001033,
	.initVal[SNAFC_SPEED_SEL_1].l = 0x00001033,
	.initVal[SNAFC_SPEED_SEL_2].l = 0x00001103,
	.initVal[SNAFC_SPEED_SEL_3].l = 0x00001103,
	.clkSel = SNAFC_SPEED_SEL_0,
...
#define DEFAULT_W_DAT_IOWIDTH (SNAFC_DIO_WIDTH)     /*@@ sa enum snandBusOpCmd_e in rtl8735b_snand.h */
...
#define DEFAULT_R_DAT_IOWIDTH (SNAFC_DIO_WIDTH)     /*@@ sa enum snafcBusWidth_e in rtl8735b_snand.h */
#else /* !SUPPORT_QUAD_BIT_MODE_W, using 1-bit mode as default */
...
void snand_init(snand_t *obj)
{
	...
	if (pglob_snafc_adaptor == NULL) {
		...
		uint32_t tDevId, tCmd, tRetVal, tData;
